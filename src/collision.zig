...
pub fn checkCircles(center1: Vector2, radius1: f32, center2: Vector2, radius2: f32) bool {
    return lib.c.CheckCollisionCircles(center1.to_c_struct(), radius1, center2.to_c_struct(), radius2);
}

/// Check collision between circle and rectangle
pub fn checkCircleRec(center: Vector2, radius: f32, rec: Rectangle) bool {
    return lib.c.CheckCollisionCircleRec(center.to_c_struct(), radius, rec);
}

/// Check if point is inside rectangle
pub fn checkPointRec(point: Vector2, rec: Rectangle) bool {
    return lib.c.CheckCollisionPointRec(point.to_c_struct(), rec);
}

/// Check if point is inside circle
pub fn checkPointCircle(point: Vector2, center: Vector2, radius: f32) bool {
    return lib.c.CheckCollisionPointCircle(point.to_c_struct(), center.to_c_struct(), radius);
}

/// Check if point is inside a triangle
pub fn checkPointTriangle(point: Vector2, p1: Vector2, p2: Vector2, p3: Vector2) bool {
    return lib.c.CheckCollisionPointTriangle(point.to_c_struct(), p1.to_c_struct(), p2.to_c_struct(), p3.to_c_struct());
}

/// Check if point is within a polygon described by slice of vertices
pub fn checkPointPoly(point: Vector2, points: []Vector2) bool {
    return lib.c.CheckCollisionPointPoly(point.to_c_struct(), points.ptr, @intCast(c_int, points.len));
}
...
pub fn checkLines(start_pos1: Vector2, end_pos1: Vector2, start_pos2: Vector2, end_pos2: Vector2) ?Vector2 {
    ...
    return if (lib.c.CheckCollisionLines(
        start_pos1.to_c_struct(),
        end_pos1.to_c_struct(),
        start_pos2.to_c_struct(),
        end_pos2.to_c_struct(),
        &collision_point.to_c_struct(),
...
pub fn checkPointLine(point: Vector2, p1: Vector2, p2: Vector2, threshold: i32) bool {
    return lib.c.CheckCollisionPointLine(point.to_c_struct(), p1.to_c_struct(), p2.to_c_struct(), threshold);
}
